 ...
 * @ignore
 */
class RenderPassDepthAwareBlurColor extends RenderPassShaderQuad {
    constructor(device, sourceTexture, horizontal) {
        ...
        this.shader = this.createQuadShader(`DepthAware${horizontal ? 'Horizontal' : 'Vertical'}BlurShader`, `${shaderChunks.screenDepthPS /* glsl */}
            ...
            void tap(inout vec3 sum, inout float totalWeight, float weight, float depth, vec2 position) {

                vec3 color = texture2D(sourceTexture, position).rgb;
            ...
            void main() {
                ...
                vec3 color = texture2D(sourceTexture, uv0 ).rgb;
                vec3 sum = color * totalWeight;
...
export { RenderPassDepthAwareBlurColor };
