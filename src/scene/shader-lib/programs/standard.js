...
class ShaderGeneratorStandard extends ShaderGenerator {
    ...
    _getUvSourceExpression(transformPropName, uVPropName, options) {
        ...
        if (
            isMainPass &&
      options.litOptions.nineSlicedMode === SPRITE_RENDERMODE_SLICED
        ) {
            expression = 'nineSlicedUv';
        } else if (
            isMainPass &&
      options.litOptions.nineSlicedMode === SPRITE_RENDERMODE_TILED
        ) {
            expression = 'nineSlicedUv';
        } else {
            if (transformId === 0) {
    ...
    _validateMapChunk(code, propName, chunkName, chunks) {
        Debug.call(() => {
            ...
            if (code.includes('$texture2DSAMPLE')) {
                trackChange(
                    '$texture2DSAMPLE',
                    '(Macro no longer supported - remove/refactor)'
                );
            }

            if (requiredChangeStrings.length > 0) {
                Debug.errorOnce(
                    `Shader chunk ${chunkName} is in no longer compatible format. Please make these replacements to bring it to the current version:\n${requiredChangeStrings.join(
                        '\n'
                    )}`,
                    { code: code }
                );
    ...
    _addMapDefines(
        ...
        if (textureOption) {
            ...
            const uv = this._getUvSourceExpression(
                transformPropName,
                uVPropName,
                options
            );
            ...
            fDefines.set(
                `{STD_${propNameCaps}_TEXTURE_CHANNEL}`,
                options[channelPropName]
            );
            ...
            if (encoding) {
                ...
                const textureDecode =
          options[channelPropName] === 'aaa' ?
              'passThrough' :
              ChunkUtils.decodeFunc(encoding);
        ...
        if (vertexColorOption) {
            ...
            fDefines.set(
                `{STD_${propNameCaps}_VERTEX_CHANNEL}`,
                options[vertexColorChannelPropName]
            );
        }
    ...
    createVertexShader(litShader, options) {
        ...
        for (const p in _matTex2D) {
            ...
            if (options[`${p}VertexColor`]) {
                ...
                options[colorChannelName] = this._correctChannel(
                    p,
                    options[colorChannelName],
                    _matTex2D
                );
            }
            ...
            if (options[mapName]) {
                ...
                options[channelName] = this._correctChannel(
                    p,
                    options[channelName],
                    _matTex2D
                );
          ...
          useUnmodifiedUv[uvSet] ||
          (options[mapName] && !options[transformName]);
        ...
        if (options.forceUv1) {
            ...
            useUv[4] = true;
            useUnmodifiedUv[1] =
        useUnmodifiedUv[1] !== undefined ? useUnmodifiedUv[1] : true;
            useUnmodifiedUv[2] =
        useUnmodifiedUv[2] !== undefined ? useUnmodifiedUv[2] : true;
            useUnmodifiedUv[3] =
        useUnmodifiedUv[3] !== undefined ? useUnmodifiedUv[3] : true;
            useUnmodifiedUv[4] =
        useUnmodifiedUv[4] !== undefined ? useUnmodifiedUv[4] : true;
        }
    ...
    prepareFragmentDefines(options, fDefines, shaderPassInfo) {
        ...
        fDefineSet(
            options.dirLightMap && options.litOptions.useSpecular,
            'STD_LIGHTMAP_DIR',
            ''
        );
        ...
        fDefineSet(
            options.aoMap || options.aoVertexColor || options.useAO,
            'STD_AO',
            ''
        );
        fDefineSet(
            ...
            'STD_OPACITY_DITHER',
            ditherNames[
            shaderPassInfo.isForward ?
                options.litOptions.opacityDither :
                options.litOptions.opacityShadowDither
            ]
        );
    }
    ...
    createShaderDefinition(device, options) {
        const shaderPassInfo = ShaderPass.get(device).getByIndex(
            options.litOptions.pass
        );
            ...
            if (litShader.needsNormal) {
                if (options.normalMap || options.clearCoatNormalMap) {
                    if (!options.litOptions.hasTangents) {
                        ...
                        const baseName = options.normalMap ?
                            'normalMap' :
                            'clearCoatNormalMap';
                        lightingUv = this._getUvSourceExpression(
                            `${baseName}Transform`,
                            `${baseName}Uv`,
                            options
                        );
